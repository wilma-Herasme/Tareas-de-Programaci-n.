...
{
char p1;
printf("\nIngrese un caracter para anlizar si este es in dígito: ");
p1 = getchar();
if (isdigit (p1))
...
if (isalpha (p1))
    if (isupper(p1))
        printf("%c fue convertirla de mayúscula a minúscula \n", tolower(p1));
        /* La funcion tolower convierte de mayuscula a minúscula. Si la
