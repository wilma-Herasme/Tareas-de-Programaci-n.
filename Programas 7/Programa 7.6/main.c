...
strcpy(cad1, cad0);
/* La funcion stcpy permite copiar una cadena de caracteres completa. En este
caso se copia la cadena cad0 a cad1. Si el espacio recervado para cad1 es
...
%s\n", cad1);
strcpy(cad1, "XX");
printf("\nPrueba de la función strcpy. Se copia la cadena XX a cadd1:
%s\n", cad1);

strncpy(cad2, cad0, 4);
cad2[4] = '\0';
/* La función strncpy permite copiar un numero determinado de caracteres a
...
reservado para cad0 es menor a la que se debe almacenar se genera un error
de ejecución. */
...
strcat(cad2, " ");
strncat(cad2, cad0, 4);
printf("\nPrueba de la función strncat. Se incorporan 4 caraceres de cad0
a cad2:
                    %s\n", cad2)
